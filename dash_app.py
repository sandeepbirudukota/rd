...
from predict import predict_sevirity
import base64
import numpy as np
...
def get_inputs_for_predictions():
    return html.Div([
            get_input('number', 'temp', 'Temperature(F)', value='30.9'),
            get_input('number', 'distance', 'Distance(mi)', value='1.585'),
            get_input('number', 'humidity', 'Humidity(%)', value='92'),
            get_input('number', 'pressure', 'Pressure(in)', value='29.26'),
            get_input('number', 'visibility', 'Visibility(mi)', value='1'),
            get_input('number', 'wind_speed', 'Wind Speed(mph)', value='19.6'),
            get_input('number', 'precipitation', 'Precipitation(in)', value='0.06'),
...
def update_output(value, temp_id, day_night, distance_id, humidity_id, pressure_id,
    ...
    city_name = city_name if city_name else 'Dayton'
    scalar_dict = {
    ...
    side_r = right_left
    Start_Time = date_selected+' 06:10:59'
    print("front_end_inputs----")
    print(value, temp_id, day_night, distance_id, humidity_id, pressure_id,
                  visibility_id, wind_speed_id, precipitation_id, start_lang_id, start_lat_id,
                  city_name, wind_direction, weather_condition, right_left, date_selected)
    print("front_end_inputs----")
    ...
    severity = predict_sevirity(scalar_dict=scalar_dict, surroundings_map=surroundings_map,Start_Time=Start_Time,
                                city_name= city_name
                                
                                )
    # severity = predict_sevirity()
    ...
    return html.Div([
                html.Div([
                        ...
                        html.Table([
                                ...
                                html.Tr([
                                        html.Th([html.Label(['xxxx'])], style={'border': '1px solid', 'textAlign': 'center'}),
                                        html.Th([html.Label(['xxxx'])], style={'border': '1px solid', 'textAlign': 'center'}),
                                        html.Th([html.Label(['xxxx@sjsu.edu'])], style={'border': '1px solid', 'textAlign': 'center'}),
                                    ]),
                                html.Tr([
                                        html.Th([html.Label(['xxxx'])], style={'border': '1px solid', 'textAlign': 'center'}),
                                        html.Th([html.Label(['xxxx'])], style={'border': '1px solid', 'textAlign': 'center'}),
                                        html.Th([html.Label(['xxxxx@sjsu.edu'])], style={'border': '1px solid', 'textAlign': 'center'}),
                                    ]),
                            ])
                    ], style={'display': 'flex','flex-direction': 'column'}, className='p-3'),
                html.Div([
